...
public class NodeSelectorSynchronizationService extends StatelessService {
    ...
    private boolean isDetailedLoggingEnabled = Configuration.bool(
    ...
    private void handleSynchronizeRequest(Operation post, SynchronizePeersRequest body) {
        ...
        boolean synchHistoricalVersions = post
        ...
        if (synchHistoricalVersions) {
            ...
            URI localQueryTasksUri = UriUtils.buildUri(getHost(),
    ...
    private void handleBroadcastQueryCompletion(QueryTask result, Operation post,
        ...
        Set<String> synchronizedVersionedLinks = new HashSet<>(
                queryResult.documentCount.intValue());
        ...
        Operation getPage = Operation.createGet(UriUtils.buildUri(getHost(), pageLink))
                .setReferer(getUri()).setCompletion((o, e) -> {
                    ...
                    post.nestCompletion((oo, ee) -> {
                        ...
                        synchronizePage(page.nextPageLink, post, request,
                                synchronizedVersionedLinks,
                                queryTaskLink);
                    });

                    synchronizeSingleVersion(page.documentLinks, 0, request, post,
                            synchronizedVersionedLinks);
                });
    ...
    private void synchronizeSingleVersion(List<String> documentLinks, int documentLinkIndex,
        ...
        if (synchronizedVersionedLinks.contains(documentLink)) {
            synchronizeSingleVersion(documentLinks, documentLinkIndex + 1, request, post,
                    synchronizedVersionedLinks);
            return;
        }
    ...
    private void createAndBroadcastGetQuery(Operation post, SynchronizePeersRequest body,
            String documentLink) {
        ...
        URI localQueryUri = UriUtils.buildDocumentQueryUri(
                ...
                body.options);
        if (version != null) {
            localQueryUri = UriUtils.extendUriWithQuery(localQueryUri,
                    ServiceDocument.FIELD_NAME_VERSION, version);
            try {
        ...
        getHost().broadcastRequest(this.parent.getSelfLink(), selfLink,
                excludeThisHostFromBroadcast,
                remoteGet);
    }
    ...
    private void handleBroadcastGetCompletion(NodeGroupBroadcastResponse rsp, Operation post,
        ...
        for (Entry<URI, String> e : rsp.jsonResponses.entrySet()) {
            ...
            if (version != null && !version.equals(peerState.documentVersion)) {
                logWarning(
                        "Invalid state from peer %s: %s. Version mismatch: expected version %s, received %d",
        ...
        if (version == null) {
            if (bestPeerRsp == null) {
                ...
                results = ServiceDocument.compare(request.state,
                        bestPeerRsp, request.stateDescription,
                        Utils.getTimeComparisonEpsilonMicros());
            }
        }
        ...
        broadcastBestState(rsp.selectedNodes, peerStates, nonRespondingPeers, post, request,
                bestPeerRsp,
                version);
    }
    ...
    private void broadcastBestState(Map<String, URI> selectedNodes,
        ...
        try {
            ...
            AtomicInteger remaining = new AtomicInteger(
                    peerStates.size() + nonRespondingPeers.size());
    ...
    private void sendSynchPost(Operation peerOp, ServiceDocument clonedState,
