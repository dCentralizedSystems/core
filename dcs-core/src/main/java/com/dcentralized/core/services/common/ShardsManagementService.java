...
import com.dcentralized.core.common.FNVHash;
...
public class ShardsManagementService extends StatefulService {
    ...
    private void init(Operation op) {
        ShardsManagementServiceState body = op.hasBody()
                ? op.getBody(ShardsManagementServiceState.class) : null;
    ...
    public void handlePatch(Operation patch) {
        ...
        CreateOrGetShardInfoRequest createOrGetShardInfoRequestBody = patch
                .getBody(CreateOrGetShardInfoRequest.class);
        ...
        patch.fail(Operation.STATUS_CODE_BAD_REQUEST, new IllegalArgumentException(errorMsg),
                errorMsg);
    }
    ...
    private void handleAddNodes(Operation patch, AddNodesRequest body,
            ShardsManagementServiceState currentState) {
    ...
    private ShardInfo allocateNodesForNewShard(String shardId,
            ShardsManagementServiceState currentState) {
        ...
        SortedMap<Integer, Set<String>> map = currentState.shardCountToNodeIdsMap;
        // we try to allocate nodes with minimum number of existing shards
        SEARCH_NODES: for (Entry<Integer, Set<String>> e : map.entrySet()) {
        ...
        for (int i = 0; i < allocatedNodes.size(); i++) {
            ...
            Set<String> nodesPerPrevShardCount = currentState.shardCountToNodeIdsMap
                    .get(prevShardCount);
            ...
            Set<String> nodesPerNextShardCount = currentState.shardCountToNodeIdsMap
                    .computeIfAbsent(
