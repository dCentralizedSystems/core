...
public class BroadcastQueryPageService extends StatelessService {
    ...
    private String startNewService(List<String> pageLinks, String prevPageLink,
            String nextPageLink) {
        URI broadcastPageServiceUri = UriUtils.buildUri(this.getHost(), UriUtils.buildUriPath(
                ServiceUriPaths.CORE_QUERY_BROADCAST_PAGE,
    ...
    public void handleDelete(Operation delete) {
        ...
        OperationJoin.create(
                this.pageLinks.stream()
                        .map(link -> Operation.createDelete(this, link).setReferer(getUri())))
                .setCompletion((ops, exs) -> {
                    if (exs != null) {
                        logWarning(
                                "Failed to delete query result pages for broadcast query result %s: %s",
                                getUri(), Utils.toString(exs));
                    }
