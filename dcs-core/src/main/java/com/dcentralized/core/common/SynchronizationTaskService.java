...
public class SynchronizationTaskService
     ...
     */
    public static final int MAX_CHILD_SYNCH_RETRY_COUNT = Configuration.integer(
     ...
     */
    private final long checkpointPeriod = Configuration.number(
            SynchronizationTaskService.class,
            "checkpointPeriod",
     ...
     */
    private final long checkpointComparisonEpsilonMicros = Configuration.number(
            SynchronizationTaskService.class,
     ...
     */
    private final long checkpointQueryLimit = Configuration.number(
            SynchronizationTaskService.class,
    ...
    public void handlePatch(Operation patch) {
        ...
        if (this.isDetailedLoggingEnabled) {
            if (SubStage.SYNCHRONIZE.equals(currentSubStage)) {
                logFine("SubStage %s, Synchronized: %d",
    ...
    private void handleCheckpointStage(State task) {
        ...
        Operation get = Operation
                ...
                .setCompletion((o, e) -> {
                    ...
                    List<Long> checkPoints = rsp.jsonResponses.values()
                            .stream()
                            .map(s -> {
                                CheckpointService.CheckpointState checkpointState = Utils.fromJson(
                                        s,
    ...
    public void handlePeriodicMaintenance(Operation maintOp) {
        ...
        long ownershipCheckExpiration = Utils
                .fromNowMicrosUtc(getHost().getOperationTimeoutMicros());
    ...
    private void getCheckpoints(Operation maintOp) {
        ...
        Operation get = Operation
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (rsp.availableNodeCount < rsp.membershipQuorum) {
                        log(Level.INFO, "availableNodeCount(%d) < membershipQuorum(%d)",
                                rsp.availableNodeCount, rsp.membershipQuorum);
                        maintOp.fail(new IllegalStateException(
                                String.format("availableNodeCount(%d) < membershipQuorum(%d)",
                                        rsp.availableNodeCount, rsp.membershipQuorum)));
                        return;
                    }
                    if (!rsp.failures.isEmpty()) {
                        for (Map.Entry<URI, ServiceErrorResponse> failure : rsp.failures
                            ...
                            if (failure.getValue().statusCode != Operation.STATUS_CODE_NOT_FOUND) {
                                log(Level.INFO, "get checkpoint failed with status %d from %s",
                                        failure.getValue().statusCode, failure.getKey());
                                maintOp.fail(new IllegalStateException(String.format(
                                        "get checkpoint failed with status %d from %s",
                                        failure.getValue().statusCode, failure.getKey())));
                                return;
                            }
                        }
        ...
        this.getHost().broadcastRequest(this.parent.getPeerNodeSelectorPath(),
                checkPointServiceLink, false, get);
    }

    private void scanCheckpoint(long lastMinCheckpoint, Map<String, URI> selectedNodes,
            Set<URI> receivers, Operation maintOp) {
        ...
        Operation.createPost(forwardingService)
                ...
                .setCompletion((o, e) -> {
                    ...
                    if (!rsp.failures.isEmpty()) {
                        for (Map.Entry<URI, ServiceErrorResponse> failure : rsp.failures
                                .entrySet()) {
                            log(Level.INFO, "Query checkpoint failed with status %d from %s",
                                    failure.getValue().statusCode, failure.getKey());
                        }
                        maintOp.fail(new IllegalStateException(
                                String.format("Query checkpoints failed: %s", rsp.failures)));
                        return;
                    }
                    if (!validateSelectedNodes(selectedNodes, rsp.selectedNodes)) {
                        log(Level.INFO, "selectedNodes miss match, expected %s, actual %s",
                                Utils.toJsonHtml(selectedNodes),
                                Utils.toJsonHtml(rsp.selectedNodes));
                        maintOp.fail(new IllegalArgumentException(
                                String.format("selectedNodes miss match, expected %s, actual %s",
                                        Utils.toJsonHtml(selectedNodes),
                                        Utils.toJsonHtml(rsp.selectedNodes))));
                        return;
                    }
                        ...
                        if (qt.results.documentCount > 0L) {
                            results.put(uri, qt.results.documents.entrySet().stream()
                                    .collect(Collectors.toMap(entry -> entry.getKey(),
                                            entry -> Utils.fromJson(entry.getValue(),
    ...
    private void updateCheckpoints(long nextCheckpoint, Set<URI> receivers, Operation maintOp) {
        ...
        log(Level.INFO, "create next checkpoint for factory %s using timestamp %d",
                this.parent.getSelfLink(), nextCheckpoint);
        ...
        for (URI peerCheckpointServiceUri : receivers) {
            String peerCheckpointFactoryLink = UriUtils
                    .getParentPath(peerCheckpointServiceUri.toString());
    ...
    private Long findNextCheckpoint(Map<URI, Map<String, ServiceDocument>> results) {
        ...
        // filter out hit link
        for (Map.Entry<String, ServiceDocument> selectedEntry : selectedResult.getValue()
                .entrySet()) {
        ...
        for (ServiceDocument d : missDocs.values()) {
            minMissTime = minMissTime == null ? d.documentUpdateTimeMicros
                    : Long.min(minMissTime, d.documentUpdateTimeMicros);
        }
    ...
    private QueryTask buildScanQueryTask(long lastMinCheckpoint) {
        QueryTask.NumericRange<Long> r = QueryTask.NumericRange.createLongRange(lastMinCheckpoint,
                Utils.getNowMicrosUtc() - this.checkpointComparisonEpsilonMicros,
                false, true);

        QueryTask.Query q = QueryTask.Query.Builder.create()
                .addFieldClause(ServiceDocument.FIELD_NAME_SELF_LINK,
                        this.parent.getSelfLink() + UriUtils.URI_PATH_CHAR
                                + UriUtils.URI_WILDCARD_CHAR,
        ...
        return QueryTask.Builder.createDirectTask()
                ...
                .orderAscending(ServiceDocument.FIELD_NAME_UPDATE_TIME_MICROS,
