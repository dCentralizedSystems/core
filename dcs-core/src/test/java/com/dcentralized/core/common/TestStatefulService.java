...
class NoBodyExampleService extends ExampleService {
...
public class TestStatefulService extends BasicReusableHostTestCase {
    ...
    public void replicationWithNullBody() throws Throwable {
        ...
        for (URI peerUri : this.host.getInProcessHostMap().keySet()) {
            ...
            URI queryUri = UriUtils.buildIndexQueryUri(indexUri, documentSelfLink, true, false,
                    null);

            ExampleServiceState state = sender.sendAndWait(Operation.createGet(queryUri),
                    ExampleServiceState.class);
            assertEquals("Counter in persisted state should be updated", Long.valueOf(1),
                    state.counter);
        }
    }
