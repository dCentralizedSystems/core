...
public class TestNodeGroupMigrationTaskService {
    ...
    public void factoryDiscovery() {
        ...
        Set<String> factoryPaths = result.results.stream().map(report -> report.factoryPath)
                .collect(toSet());
    ...
    public void requestWithFactoryLink() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Operation post = Operation.createPost(sourceNode, ExampleService.FACTORY_LINK)
                    .setBody(body);
            posts.add(post);
        }
    ...
    public void requestWithMigrationTaskServiceState() {
        ...
        URI sourceNodeGroupReference = UriUtils.buildUri(sourceNode.getUri(),
                ServiceUriPaths.DEFAULT_NODE_GROUP);
        URI destNodeGroupReference = UriUtils.buildUri(destNode.getUri(),
                ServiceUriPaths.DEFAULT_NODE_GROUP);
    ...
    private NodeGroupMigrationState postNodeGroupMigrationTaskAndWaitFinish(
            NodeGroupMigrationState requestBody) {
        VerificationHost destNode = this.destNodes.iterator().next();
        TestRequestSender sender = destNode.getTestRequestSender();

        Operation post = Operation.createPost(destNode, NodeGroupMigrationTaskService.FACTORY_LINK)
                .setBody(requestBody);
        ...
        long waitSecond = TimeUnit.MICROSECONDS
                .toSeconds(MigrationTaskService.DEFAULT_MAINTENANCE_INTERVAL_MILLIS) * 2;
    ...
    public void maxRetry() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            body.name = "foo-" + i;
            body.documentSelfLink = body.name;
            Operation post = Operation.createPost(sourceNode, FailingFactory.SELF_LINK)
                    .setBody(body);
            posts.add(post);
        }
        sender.sendAndWait(posts);
