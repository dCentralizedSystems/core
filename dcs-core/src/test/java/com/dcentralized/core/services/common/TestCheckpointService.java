...
import com.dcentralized.core.common.CheckpointService;
...
import com.dcentralized.core.common.SynchronizationTaskService;
...
public class TestCheckpointService extends BasicTestCase {
    ...
    private void setUpLocalPeerHost() throws Throwable {
        VerificationHost host = this.host.setUpLocalPeerHost(0,
    ...
    public void testCheckpointIdempotentPost() throws Throwable {
        ...
        String expectedSelfLink = UriUtils.buildUriPath(ServiceUriPaths.CHECKPOINTS,
                UriUtils.convertPathCharsFromLink(s.factoryLink));
        // start post
        this.host.getTestRequestSender().sendAndWait(Operation
                .createPost(UriUtils.buildUri(h0, ServiceUriPaths.CHECKPOINTS)).setBody(s));
        CheckpointService.CheckpointState r = this.host.getTestRequestSender().sendAndWait(
                Operation.createGet(UriUtils.buildUri(h0, expectedSelfLink)),
        ...
        this.host.getTestRequestSender().sendAndWait(Operation
                .createPost(UriUtils.buildUri(h0, ServiceUriPaths.CHECKPOINTS)).setBody(s));
        r = this.host.getTestRequestSender().sendAndWait(
                Operation.createGet(UriUtils.buildUri(h0, expectedSelfLink)),
                CheckpointService.CheckpointState.class);
        Assert.assertEquals(s.timestamp, r.timestamp);
        Assert.assertEquals(s.factoryLink, r.factoryLink);
    ...
    public void testCheckpointServiceNotSynchronized() throws Throwable {
        ...
        for (ServiceHost h : this.host.getInProcessHostMap().values()) {
            ...
            CheckpointService.CheckpointState s = this.host.getTestRequestSender().sendAndWait(op,
                    CheckpointService.CheckpointState.class);
            Assert.assertEquals(h.getId().equals(h0.getId()) == true ? checkpoint : oldCheckpoint,
                    s.timestamp.longValue());
        }
        ...
        for (ServiceHost h : this.host.getInProcessHostMap().values()) {
            ...
            CheckpointService.CheckpointState s = this.host.getTestRequestSender().sendAndWait(op,
                    CheckpointService.CheckpointState.class);
            Assert.assertEquals(!h.getId().equals(h0.getId()) ? checkpoint : oldCheckpoint,
                    s.timestamp.longValue());
        }
    }
    ...
    public void verifyCheckpointConvergence() throws Throwable {
        ...
        List<ExampleService.ExampleServiceState> exampleStates = this.host.createExampleServices(
        ...
        for (int i = 0; i < this.updateCount; i++) {
            ...
            lastUpdateExampleState = getExampleServices(exampleStates).stream()
                    .max(this.documentComparator).get();
    ...
    public void checkpointBasedSynchronizationServiceUpdate() throws Throwable {
        ...
        List<ExampleService.ExampleServiceState> exampleStates = this.host.createExampleServices(
                this.host.getPeerHost(), this.serviceCount, null, ExampleService.FACTORY_LINK);
        Map<String, ExampleService.ExampleServiceState> exampleStatesMap = exampleStates.stream()
                .collect(Collectors.toMap(s -> s.documentSelfLink, s -> s));
        ...
        ExampleService.ExampleServiceState lastUpdateExampleState = getExampleServices(
        ...
        expectedCheckpoint = exampleStates.stream().max(this.documentComparator)
                .get().documentUpdateTimeMicros;
        verifyCheckpointsConvergence(expectedCheckpoint);
        // h0 missed all additional service update
        expectedMinMissTime = exampleStates.stream().min(this.documentComparator)
                .get().documentUpdateTimeMicros;
    ...
    public void checkpointBasedSynchronizationServiceCreation() throws Throwable {
        ...
        List<ExampleService.ExampleServiceState> additionalExampleStates = this.host
                .createExampleServices(this.host.getPeerHost(), this.serviceCount, null,
        ...
        expectedMinMissTime = additionalExampleStates.stream().min(this.documentComparator)
                .get().documentUpdateTimeMicros;
        expectedCheckpoint = additionalExampleStates.stream().max(this.documentComparator)
                .get().documentUpdateTimeMicros;
        verifyCheckpointsConvergence(expectedCheckpoint);
        // restart h0 with preserved index
        restartStatefulHost(h0);
    ...
    public void nodeJoinWithEmptyIndex() throws Throwable {
        ...
        long expectedCheckpoint = getExampleServices(exampleStates).stream()
                .max(this.documentComparator).get().documentUpdateTimeMicros;
        // check point convergence
        verifyCheckpointsConvergence(expectedCheckpoint);
    }
    ...
    private void verifyCheckpointsConvergence(Long expectedCheckpoint) throws Throwable {
        this.host.waitFor("check point convergence timeout", () -> {
            ...
            if (!actualCheckpoint.equals(expectedCheckpoint)) {
                this.host.log(Level.INFO, "Expected check point %d\nActual check point %d",
                        expectedCheckpoint, actualCheckpoint);
                return false;
            }
            return true;
        });
    ...
    private void verifySynchTask(long expectedMinMissTime, long expectedMaxHitTime,
            long expectedSynchCompletionCount) {
        this.host.waitFor("synch task finish timeout", () -> {
            ...
            String synchTaskSelflink = UriUtils.buildUriPath(
                    SynchronizationTaskService.FACTORY_LINK,
                    UriUtils.convertPathCharsFromLink(ExampleService.FACTORY_LINK));
            SynchronizationTaskService.State newState = this.host.getTestRequestSender()
                    .sendAndWait(Operation.createGet(factoryOwner, synchTaskSelflink),
            ...
            boolean validCheckpoint = newState.checkpoint.equals(expectedMinMissTime - 1)
                    || newState.checkpoint.equals(expectedMaxHitTime);
            if (!validCheckpoint) {
            ...
            if (newState.synchCompletionCount != expectedSynchCompletionCount) {
                this.host.log(Level.INFO, "actual synchCount %d, expected synchCount %d",
                        newState.synchCompletionCount, expectedSynchCompletionCount);
                return false;
            }
            return true;
        });
    ...
    private List<ExampleService.ExampleServiceState> getExampleServices(
        ...
        return this.host.getTestRequestSender().sendAndWait(ops,
    ...
    private List<CheckpointService.CheckpointState> queryCheckpoints(String checkpointServiceLink) {
        ...
        List<Operation> ops = new ArrayList<>();
        for (ServiceHost h : this.host.getInProcessHostMap().values()) {
            ops.add(Operation.createPost(h, ServiceUriPaths.CORE_LOCAL_QUERY_TASKS).setBody(task)
